...
class U1652DatasetTrainUnderSample(Dataset):
    ...
    def __init__(self,
        ...
        for idx in self.ids:
            ...
            gallery_imgs = random.sample(gallery_imgs, self.undersample)
        ...
        self.samples = copy.deepcopy(self.pairs)  # 701*54=37854

    def __getitem__(self, index):

        idx, query_img_path, gallery_img_path = self.samples[index]  # 37824(batch_size:32的整数)
        ...
        r_dom = np.random.random()
        # 水平翻转图像
        if r_dom < self.prob_flip:
...
def process_image_add_edge(path):
...
class U1652DatasetTrainAddEdge(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # for query there is only one file in folder
        query_img = process_image_add_edge(query_img_path)

        gallery_img = process_image_add_edge(gallery_img_path)
...
class U1652DatasetEvalAddEdge(Dataset):
    ...
    def __getitem__(self, index):
        # self.images:['D:\\1.jpg','D:\\2.jpg',...]
        # self.sample_ids:['0000','0001',...]
        img_path = self.images[index]
        sample_id = self.sample_ids[index]

        img = process_image_add_edge(img_path)
        ...
        if self.given_sample_ids is not None:
            if sample_id not in self.given_sample_ids:
                label = -1  # sate_query:701 , drone_gallery:951 如果drone_gallery中只加载sate_query中有的数据，没有的label=-1
...
def flip_pad(img, pad_size):
    """在左边加 pad_size 宽度的镜像块，然后右边裁掉相同宽度"""
    # 取左边pad_size部分做镜像
    left_patch = img[:, :pad_size, :]
    flipped = cv2.flip(left_patch, 1)  # 水平翻转
    # 拼接在左边
    padded = np.hstack([flipped, img])
    # 裁掉右边 pad_size 宽度
    cropped = padded[:, :-pad_size, :]
    return cropped


class U1652DatasetEvalBlackPad(Dataset):
...
class U1652DatasetEvalFlipPad(Dataset):
...
class U1652DatasetTrain(Dataset):
    ...
    def __init__(self,
        ...
        for idx in self.ids:
            ...
            query_img = "{}/{}".format(self.query_dict[idx]["path"], self.query_dict[idx]["files"][0])
...
class U1652DatasetEvalVisualize(Dataset):
