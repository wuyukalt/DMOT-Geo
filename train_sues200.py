...
config.query_folder_train = config.data_folder + "/Train/" + config.meter + "/satellite"
config.gallery_folder_train = config.data_folder + "/Train/" + config.meter + "/drone"

if config.dataset == 'S200-D2S':
    config.query_folder_test = config.data_folder + "/Test/" + config.meter + "/query_drone"
    config.gallery_folder_test = config.data_folder + "/Test/" + config.meter + "/gallery_satellite"
elif config.dataset == 'S200-S2D':
    config.query_folder_test = config.data_folder + "/Test/" + config.meter + "/query_satellite"
    config.gallery_folder_test = config.data_folder + "/Test/" + config.meter + "/gallery_drone"

if __name__ == '__main__':

    model_path = "{}/{}_{}_{}_{}_{}/{}_lr-{}_loss-{}".format(config.model_path, config.dataset, config.meter,
                                                             config.backbone, config.attention, config.aggregation,
                                                             time.strftime('%m-%d-%H-%M'), config.lr, config.loss)

    if not os.path.exists(model_path):
        os.makedirs(model_path)
    shutil.copyfile(os.path.basename(__file__), "{}/train_s200.py".format(model_path))
    copyfiles2checkpoints(model_path, "s200")
