...
class Configuration:
    ...
    # Loss
    loss: str = 'InfoNCE'  # SemiTriplet, InfoNCE, WBLoss, DSSInfoNCE, SWSInfoNCE, TKSInfoNCE
    alpha: int = 10
    label_smoothing: float = 0.1
    num_negatives: int = 5
    info_loss_alpha: int = 0.5

    # Learning Rate
    lr: float = 0.0001  # 1 * 10^-4 for ViT | 1 * 10^-1 for CNN
    scheduler: str = "linear"  # "polynomial" | "cosine" | "constant" | "linear"
    warmup_epochs: int = 0
    ...
    # for better performance 启动cudnn的自动优化，提升卷积性能，输入尺寸一致时很有用
    cudnn_benchmark: bool = False
    # make cudnn deterministic  # 是否让cudnn算法确定话，保证结果可复现但速度略慢
...
if __name__ == '__main__':
    ...
    shutil.copyfile(os.path.basename(__file__), "{}/train_u1652.py".format(model_path))
    copyfiles2checkpoints(model_path, "u1652")
